...
class TestData:
    ...
    SQL_QUERIES = [
        ...
        "SELECT passenger_name FROM passenger ORDER BY CHAR_LENGTH(passenger_name) DESC LIMIT 1;",
    ...
    def _get_index(
        ...
        Args:
            ...
            mode Literal["question", "sql_query", "query_result", "answer"]: The mode in which the search works, where it is performed, by questions or by answers.
        ...
        if mode not in ("question", "sql_query", "query_result", "answer"):
            raise ValueError(
                "Unsupported mode. Use one of these: \"question\", \"sql_query\", \"query_result\", \"answer\"")
        ...
        raise ValueError(
            "Not found, check the correctness and relevance of the entered data")

    @staticmethod
    def get_data(
            provided_data: str,
            provided_data_mode: Literal["question", "sql_query", "query_result", "answer"],
            mode: Literal["question", "sql_query", "query_result", "answer"]
    ) -> str:
        """Returns a string with the required data, for example, with the `ANSWER` that was found based on the `QUESTION`.

        Args:
            provided_data str: The data provided for the search, for example, if you need to find an `ANSWER`, you can provide a `QUESTION`.
            provided_data_mode: Literal["question", "sql_query", "query_result", "answer"]: The mode of the provided data, for example, if `ANSWER` is specified, then `provided_data` is the answer.
            mode Literal["question", "sql_query", "query_result", "answer"]: Search mode, for example, the `QUESTION` mode indicates a question will be found.
        ...
        return where_to_search[mode][TestData._get_index(provided_data, provided_data_mode)]
